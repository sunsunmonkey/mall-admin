    ...
    <a-col :span="24">
      ...
      <ul class="query-handle">
        <li>
          <a-input
            style="width: 140px"
            v-model:value.trim="pageData.queryInfo.userAccount"
            placeholder="账号"
        ...
        <li>
          <a-input
            style="width: 140px"
            v-model:value.trim="pageData.queryInfo.email"
            placeholder="邮箱"
            @keyup.enter="onSearch"
          />
        </li>
        <li>
          <a-select
            style="width: 140px"
            v-model:value="pageData.queryInfo.status"
            placeholder="角色状态"
            @change="onSearch"
          >
        ...
        <template #bodyCell="{ column, text, record }">
          ...
          <template v-if="column.dataIndex === 'action'">
            <a-button
              ...
              v-auth="['Btn_Delete_Account']"
              @click="removeAccount(record._id)"
              >删除</a-button
            >
            <a-button
              type="link"
              v-if="record.status == 0"
              v-auth="['Btn_Edit_Account']"
              @click="setAccountStatus(record._id, 1)"
              >启用</a-button
            >
            <a-button
              type="link"
              ...
              v-auth="['Btn_Edit_Account']"
              @click="setAccountStatus(record._id, 0)"
              >禁用</a-button
            >
            <a-button
              type="link"
