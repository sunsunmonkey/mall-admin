...
router.post("/wxLogin", async (req, res, next) => {
  try {
    ...
    const updateInfo = await db.findOneAndUpdate(
      { openId: openid },
      { $set: { sessionKey: session_key, token, avatarUrl, userName } }
    );
    if (updateInfo) {
...
router.post("/getuserInfo", async (req, resp, next) => {
  ...
  try {
    if (token) {
      // 查询当前用户信息
      let userInfo = await db.findOne(
        { token },
        { userRoleName: 0, passWord: 0 }
      );
      ...
      if (findRoule) {
        ...
        //先找到子菜单
        let menu = await menuDb
          .find({ $or: [{ _id: { $in: roleMenu } }] })
          .sort({ sort: 1 });
        // // 在找到父级菜单
        const partentIdList = menu
          .filter((v) => v.parentId)
          .map((v) => v.parentId);
        let menuPartent = await menuDb
          .find({ $or: [{ _id: { $in: partentIdList } }] })
          .sort({ sort: 1 });
        // 过滤出是菜单类型的数据
        let menuList = menu
          .filter((v) => v.menuType == 1)
          .concat(menuPartent.filter((v) => v.menuType == 1));
        //过滤出是按钮类型的数据
...
router.post("/getAccountList", async (req, res, next) => {
  try {
    const { pageSize, pageNumber, userAccount, phone, status, email } =
      req.body;
    let queryInfo = {
      $or: [],
    };
...
router.post("/addAccount", async (req, res, next) => {
  try {
    ...
    if (
      submitRule({ userAccount, phone, email }) ||
      reqRules({ userAccount, phone }, 40)
    ) {
...
router.post("/registerAccount", async (req, res, next) => {
  try {
    ...
    if (
      submitRule({ userAccount, phone, passWord }) ||
      reqRules({ userAccount, phone }, 40)
    ) {
      return res.jsonp({
        code: 0,
